...
class  Canvas(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if self.instrument == "brush":
            ...
            self.objects.append(Line(event.position().x(), event.position().y(), event.position().x(), event.position().y()))
        elif self.instrument == "circle":
            self.objects.append(Circle(event.position().x(), event.position().y(), event.position().x(), event.position().y()))
        self.update()
